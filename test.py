...
class Test:
    def result(self,file):
        try:
            columns=['id','first_name','last_name','email','job_title','basic_salary','allowances','Gross Salary']

            # Read the .dat file into a DataFrame
            # Adjust the parameters according to your file format
            # For example, specify the delimiter and column names if necessary
            df = pd.read_csv(file, delimiter='\t', header=None,names=columns,skiprows=1)
